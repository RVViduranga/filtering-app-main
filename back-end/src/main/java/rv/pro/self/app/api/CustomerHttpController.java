...
//    @ResponseStatus(HttpStatus.BAD_REQUEST)
    ...
    public String exceptionHandler(ConstraintViolationException exp) {
        ResponseStatusException resExp = new ResponseStatusException(HttpStatus.BAD_REQUEST,
                exp.getMessage());
        exp.initCause(resExp);              // <- The hack is here
        throw resExp;
        //return String.format("<h1>Bad Request: %s</h1>", exp.getMessage());
    }
...
//        if (!colNames.stream().anyMatch(c -> c.equalsIgnoreCase(colName))) {
...
//            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Sorting Order");
//        }

        try (Connection connection = pool.getConnection()) {
//            final String COLUMN_NAME = colNames.stream()
//                    .filter(colName::equalsIgnoreCase).findFirst().get();
